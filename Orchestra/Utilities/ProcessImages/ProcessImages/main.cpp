...
int main(int argc, char* argv[])
{
	...
	wxString folderPath = _T(argv[1]); 
	folderPath = _T(folderPath + _T("/"));
	wxString imgName = _T(_T(argv[2]));
	int nbInputImages = atoi(argv[3]);
	
	wxString outputImgName;
	if(argc >= 5) outputImgName = _T(argv[4]);
	else outputImgName = imgName;

	wxString outputPath;
	if(argc >= 6) outputPath = _T(argv[5]);
	else outputPath = _T("outputFiles/");
	
	int numImg = 1;
	if(argc >= 7) numImg = atoi(argv[6]);
	
	// Open output files.
	ofstream filePoint(_T(outputPath + _T("points.txt")));
	...
	ofstream logFile(_T(outputPath + _T("log.txt")));
	if(logFile.fail()) 
	{
	...
	while(nbInputImages--)
	{
		...
		while((ptButtom = getImageSeparationPoint(addOneToPoint(lastPoint), img)).x != -1)
		{
			...
			subImg.SetRGB(0, 0, 255, 255, 255);
			subImg.SetRGB(0, h - 1, 255, 255, 255);
			subImg.SetRGB(w - 1, 0, 255, 255, 255);
			subImg.SetRGB(w - 1, h - 1, 255, 255, 255);
	
			//LINE POINTS LOOP
			int nbPoint = 0; wxPoint lP, rP;
			wxPoint lastLinePt = getLineSeparationPoint(wxPoint(0,0), subImg);
			subImg.SetRGB(lastLinePt.x, lastLinePt.y, 0, 0, 0);
			
            while((rP = getLineSeparationPoint(addOneToPoint(lastLinePt), subImg)).x != -1)
			{
				subImg.SetRGB(rP.x, rP.y, 0, 0, 0);
				lP = lastLinePt;
			...
			subImg.SaveFile(wxString::Format("outputFiles/test%s.png", formatStringWithZero(numImgSaved)), wxBITMAP_TYPE_PNG);
			lastPoint = ptButtom;
			++numImgSaved;
		}
...
{
	return folderName + fileName + formatStringWithZero(x) + _T(".png");
}
